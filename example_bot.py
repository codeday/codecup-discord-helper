...
async def load(ctx, extension):
    client.load_extension(f'cogss.{extension}') #loads the extension in the "cogs" folder
...
async def unload(ctx, extension):
    client.unload_extension(f'cogss.{extension}') #unloads the extension in the "cogs" folder
...
for filename in os.listdir('./cogss'): #loads all files (*.py)
    if filename.endswith('.py'):
        client.load_extension(f'cogss.{filename[:-3]}') #loads the file without ".py" for example: cogss.teams
